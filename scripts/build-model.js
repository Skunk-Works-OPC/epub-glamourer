...
const { pack, buildMimetype, buildContainerXml } = require('../dist/packer');
const { buildOpf } = require('../dist/builders/opf');
const { buildNav } = require('../dist/builders/nav');
const { buildNcx } = require('../dist/builders/ncx');
const { buildCoverXhtml, buildCoverSvg } = require('../dist/builders/cover');
const { buildMetadata, generateIdentifier } = require('../dist/metadata');
...
async function main() {
  ...
  const metaJson = await fs.readJson(path.join(SRC_DIR, 'metadata.json'));
  const metadata = buildMetadata({ ...metaJson, identifier: generateIdentifier() });
...
<body>
  <section epub:type="chapter">
    ...
    chapters.push({ id, filename: `${id}.xhtml`, title, xhtmlContent, epubType: 'chapter' });
  ...
  files.set(`${OPF_DIR}/cover.xhtml`, Buffer.from(
    buildCoverXhtml('images/cover.svg', metadata.title, metadata.language), 'utf8'
  ));
  ...
  const manifest = [
    ...
    ...chapters.map(ch => ({ id: ch.id, href: ch.filename, mediaType: 'application/xhtml+xml' })),
  ];
  ...
  console.log(`\n✓ Built ${path.relative(process.cwd(), OUTPUT)} (${(stat.size / 1024).toFixed(1)} KB)`);
  console.log(`  Spine order: cover(no-linear) → title-page → copyright → toc → ${chapters.map(c => c.id).join(' → ')}`);
}
